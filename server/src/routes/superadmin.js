...
superadminRouter.post('/admins', async (req, res) => {
  ...
  await sendActivationEmail(admin.email, activateUrl, admin.county);
...
superadminRouter.post('/admins/:id/disable', async (req, res) => {
...
superadminRouter.post('/admins/:id/reactivate', async (req, res) => {
  const admin = await prisma.user.findUnique({ where: { id: req.params.id } });
  if (!admin || admin.role !== 'COUNTY_ADMIN') return res.status(404).json({ ok: false, error: 'NOT_FOUND' });
  ...
  const activateUrl = `${process.env.PUBLIC_APP_URL}/activate.html?token=${encodeURIComponent(raw)}`;
  await sendActivationEmail(updated.email, activateUrl, updated.county);
...
superadminRouter.get('/admins/:id/audit', (req, res) => pagedAudit({ actorId: req.params.id }, req, res));
